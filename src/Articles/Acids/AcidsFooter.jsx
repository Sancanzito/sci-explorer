// components/Acids/AcidsFooter.jsx
...
const AcidsFooter = () => {
  ...
  const references = [
    {
      id: 1,
      citation: "OpenStax. (2024). Chemistry 2e: Acid-Base Equilibria. Rice University.",
      url: "https://openstax.org/details/books/chemistry-2e"
    },
    {
      id: 2,
      citation: "LibreTexts Chemistry. (2025). Acids, Bases, and pH. U.S. Department of Education.",
      url: "https://chem.libretexts.org/"
    },
    {
      id: 3,
      citation: "PhET Interactive Simulations. (2024). pH Scale and Acid-Base Solutions. University of Colorado Boulder.",
  ...
  const handleDownloadPDF = async () => {
    ...
    const element = document.getElementById('printable-acids-content');
    if (!element) {
    ...
    try {
      ...
      pdf.save('Acids_Bases_And_Indicators_Chapter.pdf');
    } catch (error) {
            ...
            {references.map((ref) => (
              <li key={ref.id} className="pl-1">
                ...
                <a 
                  ...
                  className="text-violet-600 dark:text-violet-400 hover:underline inline-flex items-center gap-0.5 font-medium ml-1"
                >
                  View Resource
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 border-t border-gray-100 dark:border-gray-800/60 pt-8">
          ...
          >
            {isGenerating ? (
              ...
              <>
                <Download className="w-4 h-4 text-violet-200 group-hover:text-white transition-colors" />
                Export Chapter (PDF)
              </>
            )}
          </button>
