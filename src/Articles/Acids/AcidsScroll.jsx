// components/Acids/AcidsScroll.jsx
import React from 'react';
import Scrollspy from 'react-scrollspy-navigation';
import { motion } from 'framer-motion';

const AcidsScroll = ({ sections, activeSection, onSectionClick }) => {
  return (
              ...
              return (
                <a
                  ...
                  className={`group relative flex items-center gap-3 py-2 px-2 rounded-xl text-left transition-all duration-200 w-full outline-none ${
                    isActive 
                      ? 'text-violet-600 dark:text-violet-400 bg-violet-50/60 dark:bg-violet-950/30' 
                ...
                >
                  <div className="relative flex items-center justify-center w-3 h-3 z-10">
                    {isActive && (
                      <motion.span 
                        layoutId="acidsGlowNode"
                        className="absolute w-5 h-5 bg-violet-500/20 dark:bg-violet-400/30 rounded-full blur-xs shadow-[0_0_12px_2px_rgba(139,92,246,0.4)]"
                    ...
                    <div
                      className={`w-2 h-2 rounded-full transition-all duration-200 ${
                        isActive
                          ? 'bg-violet-600 dark:bg-violet-400 scale-110 shadow-[0_0_6px_rgba(139,92,246,0.6)]'
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="acidsIndicatorBar"
                      className="absolute right-2 w-1 h-1 rounded-full bg-violet-600 dark:bg-violet-400"
